...
def View_Tweet_Message_Type_Ratio(request):

    detection_ratio.objects.all().delete()
    rratio = ""
    kword = 'Spammer Tweet'
    print(kword)
    obj = tweet_text_detection_type.objects.all().filter(Q(Prediction=kword))
    obj1 = tweet_text_detection_type.objects.all()
    count = obj.count();
    count1 = obj1.count();
    ...
    kword1 = 'Normal Tweet'
    print(kword1)
    obj1 = tweet_text_detection_type.objects.all().filter(Q(Prediction=kword1))
    obj11 = tweet_text_detection_type.objects.all()
    count1 = obj1.count();
    count11 = obj11.count();
    ...
    return render(request, 'SProvider/View_Tweet_Message_Type_Ratio.html', {'objs': obj})
...
def ViewTrendings(request):
    topic = tweet_text_detection_type.objects.values('topics').annotate(dcount=Count('topics')).order_by('-dcount')
...
def View_Prediction_Of_Tweet_Message_Type(request):
    obj =tweet_text_detection_type.objects.all()
